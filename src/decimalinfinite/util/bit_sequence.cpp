...
void BitSequence::appendBits(uint i, int n)
{
    ...
    std::cout << "Writing " << n << " - " << std::bitset<BUFFER_SIZE>(i)
              << std::endl;
#endif
    ...
    bool needs_buffer_1 =
            (_next + n - 1) / BUFFER_SIZE > (_next - 1) / BUFFER_SIZE;
#ifdef DEBUG_APPEND
...
#ifdef DEBUG_APPEND
        std::cout << "shift " << shift << std::endl;
        std::cout << "OR-ing " << std::bitset<BUFFER_SIZE>((i << shift))
                  << std::endl;
#endif
...
unsigned int BitSequence::getBits(int i, int n) const
{
    if (n > 32)
    {
        std::cout << "Error: cannot read more than 32 bits at a time (" << n
                  << " bits requested." << std::endl;
    }
#ifdef DEBUG_APPEND
...
#ifdef DEBUG_APPEND
        std::cout << "Raw bits: " << std::bitset<BUFFER_SIZE>(raw_bits)
                  << std::endl;
        std::cout << "Mask: " << std::bitset<BUFFER_SIZE>(mask) << std::endl;
        std::cout << "Raw bits, masked "
                  << std::bitset<BUFFER_SIZE>(raw_bits_masked) << std::endl;
        std::cout << "Raw bits, offset: "
                  << std::bitset<BUFFER_SIZE>(raw_bits_offset) << std::endl;
#endif
...
#ifdef DEBUG_APPEND
        std::cout << "Raw bits2: " << std::bitset<BUFFER_SIZE>(raw_bits2)
                  << std::endl;
        std::cout << "Mask2: " << std::bitset<BUFFER_SIZE>(mask2) << std::endl;
        std::cout << "Raw bits, masked2: "
                  << std::bitset<BUFFER_SIZE>(raw_bits_masked2) << std::endl;
        std::cout << "Raw bits, offset2: "
                  << std::bitset<BUFFER_SIZE>(raw_bits_offset2) << std::endl;
#endif
